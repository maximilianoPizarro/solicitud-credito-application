...
const config = defineConfig({
  ...
  publicDir: fileURLToPath(new URL('./target/classes/META-INF/resources/public', import.meta.url)),
  cacheDir: fileURLToPath(new URL('./target/.vite-cache', import.meta.url)),
  build: {
    emptyOutDir: true,
    outDir: fileURLToPath(new URL('./target/classes/META-INF/resources/', import.meta.url)),
    rollupOptions: {
      input: {
